...
public class VehicleCore implements LocalComponent {
    ...
    class CraftBookVehicleListener implements Listener {
        ...
        public void onVehicleEntityCollision(VehicleEntityCollisionEvent event) {
            ...
            if (!plugin.getConfiguration().boatRemoveEntities && !plugin.getConfiguration().minecartRemoveEntities && !plugin.getConfiguration().minecartEnterOnImpact) return;
        ...
        public void onVehicleMove(VehicleMoveEvent event) {
            ...
            if (plugin.getConfiguration().minecartConstantSpeed > 0 && RailUtil.isTrack(event.getTo().getBlock().getTypeId())
                    && event.getVehicle().getVelocity().lengthSquared() > 0) {
                Vector vel = event.getVehicle().getVelocity();
                event.getVehicle().setVelocity(vel.normalize().multiply(plugin.getConfiguration().minecartConstantSpeed));
            }
